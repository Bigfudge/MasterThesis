...
def main(mode, ocr_paths, truth_paths):
    ...
    if len(ocr_paths) != len(truth_paths):
        raise BaseException('Need equal number of files: ocr1 ocr2 [...] ocrn manual1 manual2 [...] manualn')
    else:
...
#            #print (ocrd,)
        ...
        if(totalchrs == 0 or tWCount==0):
            return(0,0)
        return(repr(float(totalchrerrs)/totalchrs), repr(float(tWErrors)/tWCount))

#main("-sb", ["./Evaluation-script/OCROutput/Ocropus/Argus/argus_lb3026335_5_0002.txt"], ["./Evaluation-script/ManuelTranscript/Argus/ed_pg_a0002.txt"], "test.txt")
